...
xVal <- function(input,output,trainFkt,predFkt, n=10){
  ...
  for(i in 1:n){ # X Validate
    ...
    #Linchen
    accuracy <- sum(predictedVals == testVals) / length(predictedVals)
    print(accuracy)
    run_Accuracy <- c(run_Accuracy, accuracy)
  ...
  #Linchen
  print(paste("Accuracy:   " ,  mean(run_Accuracy)))
...
testAVGClassification <- function(trainingSet,n=10){
  ...
  #Linchen
  predFkt <- function(classifier,data) mean(trainingSet$used)
  
  input <- trainingSet
  #output <- subset(trainingSet, select=c(coupon1Used))
  
  #Linchen
  output <- subset(trainingSet, select=c(used))
...
#Linchen, for trainingSet = melted
testSVMClassification <- function(trainingSet,n=10){
  
  trainFkt <- function(da) svm(used~., da, type="C")
...
#Linchen, for trainingSet = melted
testRFClassification <- function(trainingSet,n=10){
  
  trainFkt <- function(da) randomForest(used~., da)
  predFkt <- function(classifier,data) {as.factor(predict(classifier, data) >= 0.5)}
  input <- trainingSet
  output <- subset(trainingSet, select=c(used))
